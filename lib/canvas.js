...
class Canvas {
  ...
  draw(samples, flux, averageFlux) {
    ...
    for (let j = 0; j < samples.length; j++) {
      let sample = Math.floor(255 * (1 - Math.pow(samples[j] / 255, 0.4)))
    ...
    this.ctx.fillRect(this.size - 1, 0, 1, Math.round(50 * flux / this.size))

    this.ctx.fillStyle = 'blue'
    this.ctx.fillRect(this.size - 1, Math.round(50 * averageFlux / this.size), 1, 1)
  }
}
